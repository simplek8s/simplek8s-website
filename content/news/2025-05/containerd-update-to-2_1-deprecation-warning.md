...
{{< alert type="warning" >}}
A recent update to containerd to version 2.1.x was applied within SimpleK8s release after 202505xx. This update includes a deprecation warning that requires attention.
{{< /alert >}}

**Deprecation Warning:**

Containerd v2.1 introduced a deprecation warning related to the configuration of CNI plugins. Specifically the replace of `bin_dir` by `bin_dirs`:
...
```sh
sed -i 's/bin_dir/bin_dirs/g' /etc/containerd/config.toml
```
